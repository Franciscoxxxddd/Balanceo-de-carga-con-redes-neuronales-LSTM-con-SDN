# Reporte Técnico: Prueba 10 - Resiliencia (Estabilidad a Largo Plazo)

## 1. Descripción de la Prueba
**Objetivo:** Prueba de larga duración (15 minutos) alternando todas las condiciones para evaluar la estabilidad de memoria y decisiones a largo plazo.
...
- **Criterio Principal:** Sin caídas del controlador (OOM); mantener latencia consistente; CPU overhead manejable.
...
| **Latencia Promedio (IPTD)** | 1.20 ms | 1.11 ms | Menor es mejor. |
| **Jitter Promedio (IPDV)** | 2.27 ms | 2.28 ms | Idealmente < 5ms para VoIP. |

### 4.2 Estándar RFC 2544 (Rendimiento Físico)
| Métrica | Round Robin (RR) | IA (LSTM) | Lectura |
| :--- | :---: | :---: | :--- |
| **Throughput Promedio Servidor** | 9.82 Mbps | 9.95 Mbps | Mayor es mejor (límite: 10Mbps total). |
| **Tiempo de Recuperación (Recovery)** | 1 s | 1 s | Menor es mejor. Rápida vuelta a estabilidad. |

### 4.3 Overhead del Controlador (Consumo de Recursos)
| Recurso / Métrica | Round Robin (RR) | IA (LSTM) |
| :--- | :---: | :---: |
| **CPU Promedio** | 19.27% | 21.71% |
| **CPU Pico (Max)** | 23.60% | 27.60% |
| **RAM Promedio** | 0.70% | 6.70% |
| **Total de Decisiones Tomadas** | 20 | 254 |
| **Tiempo Respuesta por Decisión** | 55230.90 ms | 4147.35 ms |
