# Reporte Técnico: Prueba 1 - Tráfico Base (Control)

## 1. Descripción de la Prueba
**Objetivo:** Establecer una línea base de rendimiento. Tráfico ligero y constante sin congestión inducida.
...
- **Criterio Principal:** Cero pérdida de paquetes (IPLR = 0%), Latencia baja (IPTD < 5ms).
...
| **Latencia Promedio (IPTD)** | 1.07 ms | 0.94 ms | Menor es mejor. |
| **Jitter Promedio (IPDV)** | 1.65 ms | 1.83 ms | Idealmente < 5ms para VoIP. |

### 4.2 Estándar RFC 2544 (Rendimiento Físico)
| Métrica | Round Robin (RR) | IA (LSTM) | Lectura |
| :--- | :---: | :---: | :--- |
| **Throughput Promedio Servidor** | 10.07 Mbps | 10.03 Mbps | Mayor es mejor (límite: 10Mbps total). |
| **Tiempo de Recuperación (Recovery)** | 23 s | 60 s | Menor es mejor. Rápida vuelta a estabilidad. |

### 4.3 Overhead del Controlador (Consumo de Recursos)
| Recurso / Métrica | Round Robin (RR) | IA (LSTM) |
| :--- | :---: | :---: |
| **CPU Promedio** | 4.46% | 8.34% |
| **CPU Pico (Max)** | 9.80% | 23.60% |
| **RAM Promedio** | 0.70% | 6.70% |
| **Total de Decisiones Tomadas** | 20 | 27 |
| **Tiempo Respuesta por Decisión** | 9186.53 ms | 6709.37 ms |
