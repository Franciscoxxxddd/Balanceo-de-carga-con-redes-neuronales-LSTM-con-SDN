...
import json # Importar json para serializar/deserializar datos en formato JSON
import time # Importar time para obtener marcas de tiempo y medir intervalos
...
import warnings # Importar warnings para suprimir advertencias de scikit-learn durante la predicción
from ryu.base import app_manager # Importar app_manager de Ryu, la clase base para crear aplicaciones Ryu
from ryu.controller import ofp_event # Importar ofp_event para manejar eventos OpenFlow (PacketIn, SwitchFeatures, etc.)
from ryu.controller.handler import CONFIG_DISPATCHER, MAIN_DISPATCHER, set_ev_cls # Importar los decoradores para registrar handlers en fases específicas del protocolo OpenFlow
from ryu.ofproto import ofproto_v1_3 # Importar la definición del protocolo OpenFlow versión 1.3
from ryu.lib.packet import packet, ethernet, arp, ipv4, tcp, udp, ether_types # Importar clases para parsear los diferentes protocolos dentro de los paquetes de red
import tensorflow as tf # Importar TensorFlow, el framework de deep learning
from tensorflow.keras.models import load_model # Importar la función load_model de Keras para cargar el modelo LSTM entrenado

# Definición de la clase principal del controlador SDN con balanceo de carga basado en IA
class GNS3AILoadBalancer(app_manager.RyuApp):
    # Especificar que esta aplicación usa el protocolo OpenFlow versión 1.3
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    # Constructor del controlador: inicializa el modelo de IA, las VIPs y los servidores
    def __init__(self, *args, **kwargs):
        # Llamar al constructor de la clase padre RyuApp
        super(GNS3AILoadBalancer, self).__init__(*args, **kwargs)
        # Registrar mensaje de inicio del controlador en el log
        self.logger.info(">>> [INIT] Iniciando Controlador")
        
        # Bloque try-except para cargar el modelo de IA y sus dependencias
        try:
            # Cargar el modelo LSTM entrenado desde el archivo .h5
            self.model = load_model('modelo_gns3.h5')
            # Cargar el escalador MinMax para normalizar las features antes de la predicción
            self.scaler = joblib.load('scaler_gns3.joblib')
            # Cargar el encoder del target para convertir índices numéricos a nombres de servidor
            self.encoder_target = joblib.load('encoder_target_gns3.joblib')
            # Cargar el encoder de etapa de tráfico (BAJO/MEDIO/ALTO -> número)
            self.encoder_etapa = joblib.load('encoder_etapa_gns3.joblib')
            # Warmup del modelo LSTM para evitar latencia de 2s en la primera predicción
            import numpy as np
            dummy_X = np.zeros((1, 1, 12))
            self.model(dummy_X, training=False)
            self.logger.info(">>> Modelo LSTM pre-calentado exitosamente.")
            # Registrar mensaje de éxito al cargar el modelo
            self.logger.info(">>> Cerebro cargado exitosamente.")
        # Si ocurre cualquier error al cargar los archivos
        except:
            # Marcar el modelo como no disponible
            self.model = None
            # Registrar el error en el log
            self.logger.error("!!! ERROR Cargando modelo.")

        # Ruta del archivo JSON donde se exportan los datos para el dashboard en tiempo real
        self.DASHBOARD_FILE = 'estado_red.json'
        # Marca de tiempo de la última escritura al dashboard (para control de frecuencia)
        self.last_write = 0
    ...
    # Método que usa el modelo LSTM para predecir el servidor óptimo dado un conjunto de métricas
    def predecir_ia(self, metricas):
        # Si el modelo no está cargado, retornar srv1 como fallback
        if not self.model: return 'srv1'
        
        # Definir el orden exacto de las 12 features que espera el modelo
        features_order = [
            'srv1_cpu','srv1_ram', 'srv2_cpu','srv2_ram', 'srv3_cpu','srv3_ram', 'srv4_cpu','srv4_ram',
            'bw_solicitado','jitter','loss','etapa_trafico'
        ]
        
        # Si la etapa de tráfico no está en las métricas, asignar 0 (BAJO por defecto)
        if 'etapa_trafico' not in metricas:
            metricas['etapa_trafico'] = 0 # Default BAJO

        # Construir el vector de features en el orden correcto
        vector = []
        # Iterar sobre cada nombre de feature
        for f in features_order:
            # Agregar el valor de la métrica al vector (0 si no existe)
            vector.append(metricas.get(f, 0))
        
        # Suprimir advertencias de scikit-learn durante la transformación
        with warnings.catch_warnings():
            # Configurar para ignorar todas las advertencias
            warnings.simplefilter("ignore")
            # Escalar (normalizar) el vector usando el scaler entrenado
            X = self.scaler.transform([vector])
            
        # Redimensionar el array a la forma que espera el LSTM: (1 muestra, 1 timestep, 12 features)
        X = X.reshape(1, 1, 12)
        
        # Ejecutar la predicción del modelo LSTM (usar llamada directa es 10x más rápido que .predict)
        ...
        return self.encoder_target.inverse_transform([idx])[0]
    ...
    def export_dashboard(self, src, svc, chosen, mets):
        ...
        try:
            ...
            evento = {'cliente': src, 'servicio': svc, 'servidor_elegido': chosen, 'metricas': clean_mets, 'modo': 'IA'}
    ...
    def _packet_in_handler(self, ev):
        ...
        if ip_pkt:
            ...
            if ip_pkt.dst in self.SERVICES:
                ...
                # Ejecutar el balanceo de carga para este paquete
    ...
    # Método que realiza el balanceo de carga: decide qué servidor atenderá la solicitud
    def _balancear(self, datapath, msg, pkt, ip_pkt, in_port, svc):
        ...
        # Esto asegura que un cliente siempre va al mismo servidor para el mismo servicio
        ...
        if session_key in self.sticky_sessions:
            ...
            # HEALTH CHECK: Si el servidor asignado está caído o saturado (>90% CPU), hacer evicción
            metrics = self._get_live_metrics()
            if metrics.get(f'{chosen}_cpu', 0) > 90:
                old_chosen = chosen
                chosen = self.predecir_ia(metrics)
                self.sticky_sessions[session_key] = chosen
                print(f"--> [{svc['name']}] -> IA RE-eligió {chosen} (Evicción de {old_chosen} por Alta Carga)")
                self.export_dashboard(ip_pkt.src, svc['name'], chosen, metrics)
        # Si no existe sesión previa, es una nueva conexión
        else:
            # Obtener métricas simuladas del estado actual de los servidores
            metrics = self._get_live_metrics()
            # Consultar al modelo LSTM para que prediga el servidor óptimo
            chosen = self.predecir_ia(metrics)
            # Guardar la asignación en la tabla de sesiones sticky
            self.sticky_sessions[session_key] = chosen
            # Imprimir la decisión de la IA para esta nueva sesión
            print(f"--> [{svc['name']}] -> IA eligió {chosen} (Nueva Sesión: {ip_pkt.src})")
            # Exportar la decisión al dashboard para visualización en tiempo real
            self.export_dashboard(ip_pkt.src, svc['name'], chosen, metrics)
